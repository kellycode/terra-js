...
export class Terra_Terramap {
    ...
    static computeData(hf, lightDir, buf) {
        let vnorms = hf.vtxNormals;
        let w = hf.xCount + 1;
        let h = hf.yCount + 1;
        let n = Terra_Vec.Vec3.create();
        let tStart = Date.now();
        ...
        for (let y = 0; y < h; ++y) {
            for (let x = 0; x < w; ++x) {
                ...
                let light = Math.max(-Terra_Vec.Vec3.dot(n, lightDir), 0.0);
    ...
    static _v = Terra_Vec.Vec2.create();

    static computeShade(hf, lightDir, ix, iy) {
        ...
        hdir.x = -lightDir.x;
        hdir.y = -lightDir.y;
        Terra_Vec.Vec2.normalize(hdir, hdir);
        let zstep = (Terra_Vec.Vec2.length(hdir) / Terra_Vec.Vec2.length(lightDir)) * (-lightDir.z);
        let x = ix;
        let y = iy;
        ...
        while (height < hf.maxHeight) {
            ...
            y += hdir.y;
            height += zstep;
            let qx = (0, Terra_Math.pmod)(Math.round(x), w);
            let qy = (0, Terra_Math.pmod)(Math.round(y), h);
    ...
    static addWindData(imgWind, buf) {
        let canvas = document.createElement('canvas');
