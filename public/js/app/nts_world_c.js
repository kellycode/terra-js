...
class NTS_WORLD_C {
    ...
    constructor(assets, numGrassBlades, grassPatchRadius, displayWidth, displayHeight, antialias) {
        ...
        this.heightMapScale = this.vec_1.Vec3.create(
        ...
        this.meshes.sunFlare = new THREE.Mesh(
            new THREE.PlaneGeometry(6.0, 4.0, 1, 1),
            new THREE.MeshBasicMaterial({
                color: this.vec_1.Color.to24bit(this.GLARE_COLOR),
                fog: false,
                transparent: true,
                opacity: 0.0,
                depthTest: false,
        ...
        this.player = new NTS_PLAYER_C(this.heightField, this.WATER_LEVEL, this);
    ...
    updateChaseCamera(PLAYER, CHASE_CAMERA) {
        ...
        //let lr_offset = 0;//CHASE_CAMERA.userData.leftRightOffset;
        //let ud_offset = 0;//CHASE_CAMERA.userData.upDownOffset;

        // /1000 is dealing with javascript floating point
        let rotZ = Math.cos(PLAYER.rotation.y);
        let rotX = Math.sin(PLAYER.rotation.y);

        // behind player
        let distance = -50;

        CHASE_CAMERA.position.x = PLAYER.position.x - distance * rotX;
