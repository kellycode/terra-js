...
export class Terra_Heightfield {
    ...
    static HInfo() {
        return {
            i: 0, t: 0, z: 0.0, n: Terra_Vec.Vec3.create()
        };
    };

    static genFromImg(image, hf) {
    ...
    static calcFaceNormals(hf) {
        var csz = hf.cellSize, xc = hf.xCount, // tile X & Y counts
              ...
              normals = hf.faceNormals, v0 = Terra_Vec.Vec3.create(), v1 = Terra_Vec.Vec3.create(), n = Terra_Vec.Vec3.create(); // used to compute normals
        var i = 0;
        var tStart = Date.now();
        for (var iy = 0; iy < yc; ++iy) {
            for (var ix = 0; ix < xc; ++ix) {
                ...
                Terra_Vec.Vec3.cross(v0, v1, n);
                Terra_Vec.Vec3.normalize(n, n);
                normals[i + 0] = n.x;
                normals[i + 1] = n.y;
                normals[i + 2] = n.z;
    ...
    static calcVertexNormals(hf) {
        var vnorms = hf.vtxNormals;
        var w = hf.xCount + 1;
        var h = hf.yCount + 1;
        var n = Terra_Vec.Vec3.create();
        var i = 0;
        var tStart = Date.now();
        for (var y = 0; y < h; ++y) {
            for (var x = 0; x < w; ++x) {
    ...
    static computeVertexNormal(hf, vx, vy, n) {
        ...
        qx = (0, Terra_Math.pmod)(qx - 1, hf.xCount);
        ni = (qy * hf.xCount + qx) * 3 * 2 + 3;
        n.x += fnorms[ni + 0];
        n.y += fnorms[ni + 1];
        n.z += fnorms[ni + 2];
        // both tris of quad down and to the left
        qy = (0, Terra_Math.pmod)(qy - 1, hf.yCount);
    ...
    static infoAt (hf, x, y, wrap, hi) {
        ...
        if (x < ox || x >= -ox || y < oy || y >= -oy) {
            ...
            // wrap around
            x = (0, Terra_Math.pmod)(x - ox, hf.xSize) + ox;
            y = (0, Terra_Math.pmod)(y - oy, hf.ySize) + oy;
        }
