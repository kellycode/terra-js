...
import { Terra_Simplex } from "./Terra_Simplex.js";
...
export class Terra_Grass {
    constructor() {
        this.simplex_1 = Terra_Simplex.seed(0),
        this.BLADE_SEGS = 4;
    ...
    // opts set in Terra_App line 155
    createMesh(opts) {
        ...
        let lightDir = Terra_Vec.Vec3.clone(opts.lightDir);
        lightDir.z *= 0.5;
        ...
        let mat = new THREE.RawShaderMaterial({
            uniforms: {
                lightDir: {type: '3f', value: Terra_Vec.Vec3.toArray(lightDir)},
                ...
                grassColor: {type: '3f', value: Terra_Vec.Color.toArray(opts.grassColor)},
    ...
    initBladeOffsetVerts (offset, numBlades, patchRadius) {
        for (let i = 0; i < numBlades; ++i) {
            offset[i * 4 + 0] = Terra_Math.nrand() * patchRadius; // x
            offset[i * 4 + 1] = Terra_Math.nrand() * patchRadius; // y
            offset[i * 4 + 2] = 0.0; // z
    ...
    initBladeIndexVerts (vindex) {
     ...
     // @param y Y coord
     /* good
    update (mesh, time, camPos, camDir, drawPos) {
        this.mod += 1;
        //drawPos.x += this.mod;
